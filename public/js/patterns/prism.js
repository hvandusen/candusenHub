	...
	return function(){
	  ...
	  function createShape(point){
	    ...
	    for(var i=0;i<4;i++){
	    	...
	    	path.rotate(90-number)
	    }
	    path2.fillColor = prettyRaCo();//'red'//prettyRaCo()
	    ...
	    number2 = ((protrusion+1)%4+1)-1
	    number3 = ((protrusion+3)%4)
	    fill1.add(path2.segments[number2].point.add(vector))
	    fill1.add(path2.segments[number2])
	    var fill2 = new paper.Path(proPath.segments)
	    fill2.add(path2.segments[number3].point.add(vector))
	    fill2.add(path2.segments[number3].point)
